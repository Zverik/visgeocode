...
if data and len(data) > 0:
	for f in data:
		if 'lat' in f and 'lon' in f:
			cur.execute('SELECT osm_id, ST_AsGeoJSON(ST_Transform(way, 4326)), ST_Area(way) as area FROM planet_osm_polygon WHERE ST_Transform(ST_SetSRID(ST_Point(%s, %s), 4326), 900913) && way ORDER BY area', (f['lon'], f['lat']))
			result = cur.fetchone()
